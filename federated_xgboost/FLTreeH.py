...
class H_FLXGBoostClassifierBase():
    ...
    def boost(self): # TODO MAKE for Horizondtal
        ...
        for i in range(self.nTree): 
            ...
            self.trees[i].fit_fed(y, y_pred, dataFit) # TODO make Horizontal version to do gradients
...
class H_FLPlainXGBoostTree():
    ...
    def V_tmp_fit_fed(self, y, yPred, qDataBase: QuantiledDataBase):
        logger.info("Tree is growing column-wise. Current column: %d", self.treeID)
    ...
    def fed_grow(self, qDataBase: QuantiledDataBase, depth, NodeDirection = TreeNodeType.ROOT, currentNode : FLTreeNode = None):
        ...
        if(sInfo.isValid):      
            ...
            if (depth < maxDepth) and (sInfo.bestSplitScore > 0):
                ...
                logger.info("Reached max-depth or Gain fed_predictis negative. Terminate the tree growing, generate the leaf with weight Leaf Weight: %f", currentNode.weight)
        else:
            weight, score = FLTreeNode.compute_leaf_param(qDataBase.gradVec, qDataBase.hessVec, XgboostLearningParam.LAMBDA)
            currentNode.weight = weight
            currentNode.score = score
